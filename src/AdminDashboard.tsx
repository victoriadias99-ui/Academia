...
const Toast = ({ message, type, onClose }: { message: string, type: 'success' | 'error', onClose: () => void }) => {
  ...
  return (
    <motion.div 
      ...
      exit={{ opacity: 0, y: 50 }}
      className={`fixed bottom-8 right-8 px-6 py-3 rounded-lg shadow-lg flex items-center gap-3 z-[100] ${
        type === 'success' ? 'bg-[#00a86b] text-white' : 'bg-red-600 text-white'
      }`}
    >
...
const Modal = ({ isOpen, onClose, title, children }: { isOpen: boolean, onClose: () => void, title: string, children: React.ReactNode }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 overflow-y-auto">
      ...
      >
        <div className="flex items-center justify-between p-6 border-bottom border-[#dee2e6]">
...
export default function AdminDashboard() {
  ...
  const fetchStudents = async (query = "") => {
    const res = await fetch(`/api/admin/usuarios?buscar=${query}`);
    const data = await res.json();
  ...
  const handleUpdateSubscription = async (email: string, meses?: number, activo?: boolean) => {
    try {
      const res = await fetch('/api/admin/usuarios/suscripcion', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, meses, activo })
      });
      if (res.ok) {
  ...
  const handleUpdateStudent = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/admin/usuarios/${editingStudent.email}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteStudent = async (email: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/usuarios/${email}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  const handleCreateCourse = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setToast({ message: editingCourse ? "✓ Actualizado correctamente" : "✓ Guardado correctamente", type: 'success' });
  ...
  const handleCreateLesson = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...lessonForm, cursoId: selectedCourseId })
      });
      if (res.ok) {
        setToast({ message: editingLesson ? "✓ Actualizada correctamente" : "✓ Guardada correctamente", type: 'success' });
  ...
  const handleDeleteLesson = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/admin/lecciones/${id}`, { method: 'DELETE' });
      if (res.ok) {
        setToast({ message: "✓ Lección eliminada", type: 'success' });
        if (selectedCourseId) fetchLessons(selectedCourseId);
      } else {
        setToast({ message: "Error al eliminar", type: 'error' });
      }
      ...
      <aside className="w-[240px] bg-[#1a5c4a] flex flex-col flex-shrink-0">
          ...
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-md transition-all duration-200 group ${
                activeTab === item.id 
                  ? "bg-[#00a86b]/20 border-l-[3px] border-[#00a86b] text-white" 
        ...
        <div className="p-6 mt-auto">
          <div className="flex items-center gap-3 mb-4 overflow-hidden">
            <div className="w-8 h-8 rounded-full bg-[#00a86b] flex items-center justify-center text-white font-bold text-xs shrink-0 overflow-hidden">
              {user?.foto_url ? (
          ...
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-md border border-white/20 text-white hover:bg-red-600 hover:border-red-600 transition-all duration-200"
          >
            <LogOut size={18} />
        ...
        <AnimatePresence mode="wait">
          {activeTab === "dashboard" && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div className="bg-white p-5 rounded-lg border border-[#dee2e6] shadow-sm">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-gray-500 font-medium">Total Alumnos</span>
                    <Users className="text-[#00a86b]" size={20} />
                  </div>
                  <div className="text-3xl font-bold text-[#0d2137]">{stats?.totalAlumnos}</div>
                </div>
                <div className="bg-white p-5 rounded-lg border border-[#dee2e6] shadow-sm">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-gray-500 font-medium">Total Ventas</span>
                    <DollarSign className="text-[#00a86b]" size={20} />
                  </div>
                  <div className="text-3xl font-bold text-[#0d2137]">{stats?.totalVentas}</div>
                </div>
                <div className="bg-white p-5 rounded-lg border border-[#dee2e6] shadow-sm">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-gray-500 font-medium">Ingresos USD</span>
                    <DollarSign className="text-[#00a86b]" size={20} />
                  </div>
                  <div className="text-3xl font-bold text-[#0d2137]">${stats?.ingresosUSD.toLocaleString()}</div>
                </div>
                <div className="bg-white p-5 rounded-lg border border-[#dee2e6] shadow-sm">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-gray-500 font-medium">Cursos Activos</span>
                    <BookOpen className="text-[#00a86b]" size={20} />
                  </div>
                  <div className="text-3xl font-bold text-[#0d2137]">{stats?.cursosActivos}</div>
                </div>
              </div>

              <div className="bg-white rounded-lg border border-[#dee2e6] shadow-sm overflow-hidden">
                <div className="p-6 border-b border-[#dee2e6]">
                  <h2 className="text-lg font-bold text-[#0d2137]">Últimas compras</h2>
                </div>
          ...
          {activeTab === "alumnos" && (
            ...
            >
              <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                  <h1 className="text-3xl font-bold text-[#0d2137]">Alumnos</h1>
                  <p className="text-gray-500">Gestión de estudiantes registrados</p>
                </div>
                <div className="flex gap-2">
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="pl-10 pr-4 py-2 rounded-md border border-[#dee2e6] focus:outline-none focus:ring-2 focus:ring-[#00a86b]/50 w-full md:w-64"
                      value={searchQuery}
                  ...
                  <button 
                    ...
                    className="bg-[#00a86b] text-white px-6 py-2 rounded-md font-medium hover:bg-[#008f5a] transition-colors"
                  >
                    Buscar
                  </button>
                </div>
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-[#1a5c4a] text-white">
                      <tr>
                        <th className="px-6 py-4 font-semibold">Email</th>
                        <th className="px-6 py-4 font-semibold">Nombre</th>
                        <th className="px-6 py-4 font-semibold">Cursos</th>
                        ...
                        <tr key={i} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${
                              student.activo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                            }`}>
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              <button 
                                onClick={() => {
                                  ...
                                  setStudentForm({
                                    ...
                                    vencimiento: student.vencimiento
                                  });
                              ...
                              <button 
                                onClick={() => handleUpdateSubscription(student.email, undefined, !student.activo)}
                                className={`p-1.5 rounded border transition-all ${
                                  student.activo 
                                    ? 'border-red-200 text-red-600 hover:bg-red-50' 
                                    : 'border-green-200 text-green-600 hover:bg-green-50'
                                }`}
                                title={student.activo ? "Dar de baja" : "Dar de alta"}
                              >
                                {student.activo ? <ShieldX size={16} /> : <ShieldCheck size={16} />}
                              </button>
                              <div className="flex bg-gray-100 rounded p-1">
                                {[1, 3, 12].map(m => (
                                  <button
                                    key={m}
                                    onClick={() => handleUpdateSubscription(student.email, m)}
                                    className="px-2 py-1 text-[10px] font-bold hover:bg-white rounded transition-all text-[#1a5c4a]"
                                    title={`Activar por ${m} meses`}
                                  >
                                    {m}M
                                  </button>
                                ))}
          ...
          {activeTab === "cursos" && (
            ...
            >
              <header className="flex items-center justify-between mb-8">
                <div>
                  <h1 className="text-3xl font-bold text-[#0d2137]">Cursos</h1>
                  <p className="text-gray-500">Administración de la oferta académica</p>
                </div>
                <button 
                  onClick={() => {
                    setEditingCourse(null);
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead className="bg-[#1a5c4a] text-white">
                      <tr>
                        <th className="px-6 py-4 font-semibold">Nombre</th>
                        <th className="px-6 py-4 font-semibold">Academia</th>
                        <th className="px-6 py-4 font-semibold">Stripe Price ID</th>
                        <th className="px-6 py-4 font-semibold">Precio ARS</th>
                    ...
                    <tbody className="divide-y divide-[#dee2e6]">
                      {courses.map((course) => (
                        <tr key={course.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${
                              course.activo !== false ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                            }`}>
          ...
          {activeTab === "lecciones" && (
            ...
            >
              <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                  <h1 className="text-3xl font-bold text-[#0d2137]">Lecciones</h1>
                  <p className="text-gray-500">Contenido de los cursos</p>
                </div>
                <div className="flex gap-2">
                  <select 
                    className="px-4 py-2 rounded-md border border-[#dee2e6] focus:outline-none focus:ring-2 focus:ring-[#00a86b]/50 bg-white"
                    ...
                    <table className="w-full text-left">
                      <thead className="bg-[#1a5c4a] text-white">
                        <tr>
                          ...
                          <th className="px-6 py-4 font-semibold">Vimeo ID</th>
                      ...
                      <tbody className="divide-y divide-[#dee2e6]">
                        {lessons.map((lesson, i) => (
                          <tr key={lesson.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-6 py-4">
                              {lesson.preview ? (
                                <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded-full text-[10px] font-bold uppercase">Sí</span>
                              ) : (
                                <span className="text-gray-300 text-[10px] font-bold uppercase">No</span>
                              )}
                            </td>
                            <td className="px-6 py-4">
                              <div className="flex gap-2">
          ...
          {activeTab === "ventas" && (
            ...
            >
              <header className="mb-8">
                <h1 className="text-3xl font-bold text-[#0d2137]">Ventas</h1>
                <p className="text-gray-500">Historial de transacciones</p>
              </header>
        ...
        <form onSubmit={handleCreateCourse} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <select 
                className="w-full px-3 py-2 rounded-md border border-[#dee2e6] focus:outline-none focus:ring-2 focus:ring-[#00a86b]/50"
                value={courseForm.academia}
                onChange={e => setCourseForm({...courseForm, academia: e.target.value})}
              >
                <option value="Aprende Excel">Aprende Excel</option>
          ...
          <div className="space-y-1">
            ...
            <input 
              ...
              onChange={e => setCourseForm({...courseForm, imagen_url: e.target.value})}
            />
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-1">
              <label className="text-sm font-medium text-gray-700">Stripe Price ID</label>
              <input 
                type="text" 
                className="w-full px-3 py-2 rounded-md border border-[#dee2e6] focus:outline-none focus:ring-2 focus:ring-[#00a86b]/50"
            ...
            <div className="space-y-1">
              ...
              <input 
                type="number" 
                className="w-full px-3 py-2 rounded-md border border-[#dee2e6] focus:outline-none focus:ring-2 focus:ring-[#00a86b]/50"
                value={courseForm.precio_ars}
                onChange={e => setCourseForm({...courseForm, precio_ars: Number(e.target.value)})}
              />
            </div>
            <div className="space-y-1">
          ...
          <div className="space-y-1">
            ...
            <input 
              ...
              onChange={e => setCourseForm({...courseForm, orden: Number(e.target.value)})}
            />
          </div>
          <div className="flex justify-end gap-3 pt-4">
            <button 
              ...
              className="px-6 py-2 rounded-md font-medium text-gray-500 hover:bg-gray-100 transition-colors"
            >
              Cancelar
            </button>
            <button 
        ...
        <form onSubmit={handleUpdateStudent} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <input 
              ...
              required
              disabled
              className="w-full px-3 py-2 rounded-md border border-[#dee2e6] focus:outline-none focus:ring-2 focus:ring-[#00a86b]/50 bg-gray-50"
              value={studentForm.email}
            />
          </div>
          <div className="space-y-1">
            <label className="text-sm font-medium text-gray-700">Cursos (separados por |)</label>
            <input 
              type="text" 
              className="w-full px-3 py-2 rounded-md border border-[#dee2e6] focus:outline-none focus:ring-2 focus:ring-[#00a86b]/50"
              value={studentForm.cursos}
              onChange={e => setStudentForm({...studentForm, cursos: e.target.value})}
              placeholder="excel-basico|excel-avanzado"
            />
          </div>
          <div className="space-y-1">
            <label className="text-sm font-medium text-gray-700">Vencimiento (YYYY-MM-DD)</label>
            <input 
              type="date" 
              className="w-full px-3 py-2 rounded-md border border-[#dee2e6] focus:outline-none focus:ring-2 focus:ring-[#00a86b]/50"
              value={studentForm.vencimiento}
              onChange={e => setStudentForm({...studentForm, vencimiento: e.target.value})}
            />
          </div>
          <div className="flex items-center gap-2">
            <input 
              ...
              onChange={e => setStudentForm({...studentForm, activo: e.target.checked})}
            />
            <label htmlFor="student-active" className="text-sm font-medium text-gray-700">Activo</label>
          </div>
          <div className="flex justify-end gap-3 pt-4">
            <button 
              type="button"
      ...
      >
        <form onSubmit={handleCreateLesson} className="space-y-4">
          <div className="space-y-1">
            ...
            <select 
              disabled
              className="w-full px-3 py-2 rounded-md border border-[#dee2e6] bg-gray-50 text-gray-500"
              value={selectedCourseId || ""}
            >
              {courses.map(c => (
                <option key={c.id} value={c.id}>{c.nombre}</option>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-sm font-medium text-gray-700">Vimeo ID (solo número)</label>
              <input 
                type="text" 
                required
                className="w-full px-3 py-2 rounded-md border border-[#dee2e6] focus:outline-none focus:ring-2 focus:ring-[#00a86b]/50"
            ...
            <div className="space-y-1">
              ...
              <input 
                ...
                onChange={e => setLessonForm({...lessonForm, duracion: Number(e.target.value)})}
              />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input 
                ...
                onChange={e => setLessonForm({...lessonForm, orden: Number(e.target.value)})}
              />
            </div>
            <div className="flex items-center gap-2 pt-6">
              <input 
                type="checkbox" 
                id="is_preview"
                className="w-4 h-4 text-[#00a86b] focus:ring-[#00a86b] border-gray-300 rounded"
                checked={lessonForm.preview}
                onChange={e => setLessonForm({...lessonForm, preview: e.target.checked})}
              />
              <label htmlFor="is_preview" className="text-sm font-medium text-gray-700">Es preview</label>
            </div>
          </div>
          <div className="flex justify-end gap-3 pt-4">
            <button 
